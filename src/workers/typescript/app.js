...
const { analyzeTypeScript } = require('./analyzer')
...
app.post('/process', async (req, res) => {
  ...
  try {
    ...
    const analysis = analyzeTypeScript(code, {
      objectKey,
    ...
    console.error('TypeScript worker failed:', error)
...
async function bootstrap() {
  ...
  const port = 4002
  app.listen(port, () => {
    console.log(`TypeScript worker listening on ${port}`)
  })
}

bootstrap().catch(error => {
  console.error('TypeScript worker bootstrap failed:', error)
  process.exit(1)
})
